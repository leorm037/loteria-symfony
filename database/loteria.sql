...
CREATE TABLE IF NOT EXISTS `loteria`.`bolao` (
  ...
  `planilha_jogos_csv_id` INT NULL,
  `comprovante_jogos_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_bolao_concurso_idx` (`concurso_id` ASC),
  INDEX `fk_bolao_usuario_idx` (`usuario_id` ASC),
  INDEX `fk_bolao_arquivo_planilha_jogos_csv_idx` (`planilha_jogos_csv_id` ASC),
  INDEX `fk_bolao_arquivo_comprovante_jogos_pdf_idx` (`comprovante_jogos_id` ASC),
  ...
  CONSTRAINT `fk_bolao_arquivo1`
    FOREIGN KEY (`planilha_jogos_csv_id`)
    REFERENCES `loteria`.`arquivo` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_bolao_arquivo2`
    FOREIGN KEY (`comprovante_jogos_id`)
    REFERENCES `loteria`.`arquivo` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `loteria`.`apostador` (
  ...
  `comprovante_pagamento_id` INT NOT NULL,
  `pix` VARCHAR(180) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_apostador_bolao_idx` (`bolao_id` ASC),
  INDEX `fk_apostador_arquivo_idx` (`comprovante_pagamento_id` ASC),
  ...
  CONSTRAINT `fk_apostador_arquivo`
    FOREIGN KEY (`comprovante_pagamento_id`)
    REFERENCES `loteria`.`arquivo` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
